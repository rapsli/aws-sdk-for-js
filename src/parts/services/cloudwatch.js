...
AmazonCloudWatch.prototype = {
  ...
  /**
	 ...
	 *
	 * @param {String|array} [alarm_names] A list of alarms to be deleted. Pass a string for a single value, or an indexed array for multiple values.
  ...
  /**
	 ...
	 * @param {String} [namespace] The namespace of the metric. [Constraints: The value must be between 1 and 255 characters, and must match the following regular expression pattern: <code>[^:].*</code>]
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
  ...
  /**
	 ...
	 * @param {String} [namespace] The namespace of the metric. [Constraints: The value must be between 1 and 255 characters, and must match the following regular expression pattern: <code>[^:].*</code>]
	 * @param {String} [metric_name] The name of the metric.
	 * @param {String} [start_time] The time stamp to use for determining the first datapoint to return. The value specified is inclusive; results include datapoints with the time stamp specified. <p class="note">The specified start time is rounded down to the nearest value. Datapoints are returned for start times up to two weeks in the past. Specified start times that are more than two weeks in the past will not return datapoints for metrics that are older than two weeks.</p> May be passed as a number of seconds since UNIX Epoch, or any string compatible with <php:strtotime()>.
	 * @param {String} [end_time] The time stamp to use for determining the last datapoint to return. The value specified is exclusive; results will include datapoints up to the time stamp specified. May be passed as a number of seconds since UNIX Epoch, or any string compatible with <php:strtotime()>.
	 * @param {Number} [period] The granularity, in seconds, of the returned datapoints. <code>Period</code> must be at least 60 seconds and must be a multiple of 60. The default value is 60.
	 * @param {String|array} [statistics] The metric statistics to return. Pass a string for a single value, or an indexed array for multiple values.
  ...
  /**
	 ...
	 * </p>
	 *
	 * @param {String} [alarm_name] The descriptive name for the alarm. This name must be unique within the user's AWS account
	 * @param {String} [metric_name] The name for the alarm's associated metric.
	 * @param {String} [namespace] The namespace for the alarm's associated metric. [Constraints: The value must be between 1 and 255 characters, and must match the following regular expression pattern: <code>[^:].*</code>]
	 * @param {String} [statistic] The statistic to apply to the alarm's associated metric. [Allowed values: <code>SampleCount</code>, <code>Average</code>, <code>Sum</code>, <code>Minimum</code>, <code>Maximum</code>]
	 * @param {Number} [period] The period in seconds over which the specified statistic is applied.
	 * @param {Number} [evaluation_periods] The number of periods over which data is compared to the specified threshold.
	 * @param {Double} [threshold] The value against which the specified statistic is compared.
	 * @param {String} [comparison_operator] The arithmetic operation to use when comparing the specified <code>Statistic</code> and <code>Threshold</code>. The specified <code>Statistic</code> value is used as the first operand. [Allowed values: <code>GreaterThanOrEqualToThreshold</code>, <code>GreaterThanThreshold</code>, <code>LessThanThreshold</code>, <code>LessThanOrEqualToThreshold</code>]
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
  ...
  /**
	 ...
	 * @param {Object} [metric_data] A list of data describing the metric. <ul>
	 * 	<li><code>x</code> - <code>array</code> - Optional - This represents a simple array index. <ul>
  ...
  /**
	 ...
	 * actual state.
	 *
	 * @param {String} [alarm_name] The descriptive name for the alarm. This name must be unique within the user's AWS account. The maximum length is 255 characters.
	 * @param {String} [state_value] The value of the state. [Allowed values: <code>OK</code>, <code>ALARM</code>, <code>INSUFFICIENT_DATA</code>]
	 * @param {String} [state_reason] The reason that this alarm is set to this specific state (in human-readable text format)
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
