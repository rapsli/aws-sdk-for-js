...
AmazonSDB.prototype = {
  ...
  /**
	 ...
	 * </ul>
	 *
	 * @param {String} [domain_name] The name of the domain in which the attributes are being deleted.
	 * @param {Object} [item_keypairs] Associative array of parameters which are treated as item-key-value and item-key-multivalue pairs (i.e. a key can have one or more values; think tags). <ul>
  ...
  /**
	 ...
	 * 	</li></ul>
	 * @param {Boolean|array} [replace] Whether to replace a key-value pair if a matching key already exists. Supports either a boolean (which affects ALL key-value pairs) or an indexed array of key names (which affects only the keys specified). Defaults to boolean <code>false</code>.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>Item</code> - <code>array</code> - Optional - A list of items on which to perform the operation. <ul>
	 * 		<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 			<li><code>ItemName</code> - <code>string</code> - Optional - This is the parameter format supported by the web service API. This is the item name to use.</li>
  ...
  /**
	 ...
	 *
	 * @param {String} [domain_name] The name of the domain to create. The name can range between 3 and 255 characters and can contain the following characters: a-z, A-Z, 0-9, '_', '-', and '.'.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
  ...
  /**
	 ...
	 *
	 * @param {String} [domain_name] The name of the domain in which the attributes are being deleted.
	 * @param {String} [item_name] The name of the base item which will contain the series of keypairs.
	 * @param {Object} [attributes] Similar to columns on a spreadsheet, attributes represent categories of data that can be assigned to items. Takes an associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>Attribute</code> - <code>array</code> - Optional -  This is the parameter format supported by the web service API. This is the attribute node.<ul>
	 * 		<li><code>x</code> - <code>array</code> - This represents a simple array index. <ul>
	 * 			<li><code>Name</code> - <code>string</code> - Required - The name of the attribute. </li>
	 * 			<li><code>AlternateNameEncoding</code> - <code>string</code> - Optional - This is the parameter format supported by the web service API. This is the alternate name encoding to use.</li>
  ...
  /**
	 ...
	 *
	 * @param {String} [domain_name] The name of the domain for which to display the metadata of.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
	 * 	<li><code>curlopts</code> - <code>array</code> - Optional - A set of values to pass directly into <code>curl_setopt()</code>, where the key is a pre-defined <code>CURLOPT_*</code> constant.</li>
	 * 	<li><code>returnCurlHandle</code> - <code>boolean</code> - Optional - A private toggle specifying that the cURL handle be returned rather than actually completing the request. This toggle is useful for manually managed batch requests.</li></ul>
	 * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
  ...
  /**
	 ...
	 * returned.
	 *
	 * @param {String} [domain_name] The name of the domain in which to perform the operation.
	 * @param {String} [item_name] The name of the base item which will contain the series of keypairs.
	 * @param {String|array} [attribute_name] The names of the attributes. Pass a string for a single value, or an indexed array for multiple values.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
  ...
  /**
	 ...
	 * @param {String} [domain_name] The name of the domain in which the attributes are being deleted.
	 * @param {String} [item_name] The name of the base item which will contain the series of keypairs.
	 * @param {Object} [keypairs] Associative array of parameters which are treated as key-value and key-multivalue pairs (i.e. a key can have one or more values; think tags). <ul>
	 * 	<li><code>[key]</code> - <code>array</code> - Set the custom key name as the key for this value. For the value, pass a string for a single value, or an indexed array for multiple values.</li>
	 * </ul>
	 * @param {Boolean|array} [replace] Whether to replace a key-value pair if a matching key already exists. Supports either a boolean (which affects ALL key-value pairs) or an indexed array of key names (which affects only the keys specified). Defaults to boolean <code>false</code>.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
  ...
  /**
	 ...
	 *
	 * @param {String} [select_expression] The expression used to query the domain.
	 * @param {Object} [opt] An associative array of parameters that can have the following keys: <ul>
